...
if st.button("Lakukan Prediksi"):
    if all(hero != "Choose an option" for hero in your_team + opponent_team):
        ...
        your_team_winrate = sum(df[df["Hero"].isin(your_team)]["Win"]) / sum(df[df["Hero"].isin(your_team)]["Pick"])
        opponent_team_winrate = sum(df[df["Hero"].isin(opponent_team)]["Win"]) / sum(df[df["Hero"].isin(opponent_team)]["Pick"])
